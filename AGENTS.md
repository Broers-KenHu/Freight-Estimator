...
- ERP estimate source for Invoice Reconciliation:
  - Preferred ERP Est. comparison value now comes from `data_raw.lsp.lsp_booking_order.freight`, synced locally as `LspBookingOrderSnapshot.freight`.
  - Do not use `erp_match_results.erp_carrier_freight` or `HistoricalOrder.postage_shipping_estimated_amount` as the Invoice Reconciliation ERP Est. source unless the user explicitly changes this again.
...
- InvoiceReader:
  ...
  - `InvoiceReconciliationItem.actual_freight` for the preferred path is `InvoiceOrderMatchSnapshot.amount_inc_gst` from InvoiceReader. `estimated_freight` now uses matched `LspBookingOrderSnapshot.freight` with basis `LSP_BOOKING_ORDER_FREIGHT`; this value is used directly for comparison and is not multiplied by GST. `erp_match_results.erp_carrier_freight` is kept only as source-detail trace data in the Review drawer.
  - GST basis check on 2026-06-25: local `actual_freight` equals InvoiceReader `detail_amount_inc_gst` for 863,436 mapped rows; it must be labelled/displayed as Actual inc GST. User correction on 2026-06-25: ERP Est data source is `data_raw.lsp.lsp_booking_order.freight`.
  ...
  - ERP estimate scope for Invoice Reconciliation is now LSP booking freight by tracking/order/reference match. The rebuild command first matches `InvoiceOrderMatchSnapshot.tracking_no` to `LspBookingOrderSnapshot.tracking_no`, then local order, then order/reference/shipment identity candidates. Rows without matched LSP booking freight should remain unmatched rather than falling back to old ERP estimate fields.
  ...
  - LSP booking freight sync command: `manage.py sync_lsp_booking_orders --full --batch-size 5000` for first load, then `manage.py sync_lsp_booking_orders --batch-size 5000` for incremental loads. It imports `data_raw.lsp.lsp_booking_order.freight` and related tracking/order/reference fields into `LspBookingOrderSnapshot`.
  - New preferred reconciliation command: `manage.py sync_reconciliation_snapshots`. Default flow imports InvoiceReader charge snapshots for traceability, imports `InvoiceOrderMatchSnapshot` from `dbo.erp_match_results`, then creates `InvoiceReconciliationBatch` / `InvoiceReconciliationItem` from InvoiceReader's mapped ERP/order result and local `LspBookingOrderSnapshot.freight`.
  ...
  - On 2026-06-25 LSP booking freight was fully synced: 236,796 `LspBookingOrderSnapshot` rows, 204,031 linked to local orders. Reconciliation was rebuilt as batch id `251`: 900,934 rows, all basis `LSP_BOOKING_ORDER_FREIGHT`, 124,149 rows with matched LSP booking row, status counts MATCHED/OK 50,161, EXCEPTION/OVERCHARGE 45,894, EXCEPTION/UNDERCHARGE 27,500, UNMATCHED 777,379.
