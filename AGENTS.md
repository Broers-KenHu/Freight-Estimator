...
- Main operational sync command: `manage.py sync_operational_data`.
  ...
  - It runs ERP/LSP/WMS master sync, ERP order/manual-order sync, LSP package-estimate/API quote/log sync, removes non-ERP legacy `HistoricalOrder` rows by default, and opens all active platform/warehouse/carrier-service relationships.
...
- LSP/ERP estimate source for Invoice Reconciliation:
  - Preferred comparison value is package-level LSP estimated freight, synced into `LspPackageEstimateSnapshot.lsp_estimated_freight`.
  - Source bridge: `data_raw.wms.doc_order_details.dedi01` is the consignment/package code and joins to `data_raw.lsp.lsp_booking_order_package.package_code`, then to `data_raw.lsp.lsp_booking_order`.
  - Estimate rule priority: use nonzero `lsp_booking_order_package.freight`; otherwise use nonzero `lsp_quote_task_package.predict_price` summed by package; only use `lsp_booking_order.freight` when that booking has exactly one package.
  - Do not use `fuel_surcharge_inclu_gst`, invoice-level fuel, `erp_match_results.erp_carrier_freight`, or `HistoricalOrder.postage_shipping_estimated_amount` as the Invoice Reconciliation preferred estimate unless the user explicitly changes this again.
  - `LspBookingOrderSnapshot.freight` is kept for trace/history and API quote context, but it is not the authoritative reconciliation estimate when package snapshots exist.
  - For system/rate-card estimates, still quote each tracking/consignment when needed, aggregate back to owner-order level, then compare with the chosen source estimate.
...
- InvoiceReader:
  ...
  - `InvoiceReconciliationItem.actual_freight` for the preferred path is `InvoiceOrderMatchSnapshot.amount_inc_gst` from InvoiceReader. `estimated_freight` now uses matched `LspPackageEstimateSnapshot.lsp_estimated_freight` aggregate with basis `LSP_PACKAGE_ESTIMATE_FREIGHT`; this value is used directly for comparison and is not multiplied by GST. `erp_match_results.erp_carrier_freight` is kept only as source-detail trace data in the Review drawer.
  - GST basis check on 2026-06-25: local `actual_freight` equals InvoiceReader `detail_amount_inc_gst` for 863,436 mapped rows; it must be labelled/displayed as Actual inc GST. User correction on 2026-06-25: do not compare with invoice-level fuel/whole-invoice values; use WMS `dedi01` package bridge into LSP package estimates.
  ...
  - Estimate scope for Invoice Reconciliation is now LSP package freight by tracking/package bridge. The rebuild command first matches `InvoiceOrderMatchSnapshot.tracking_no` to `LspPackageEstimateSnapshot.tracking_no`, then falls back to package/order identity only when the invoice row has no tracking. Rows without matched package estimate should remain unmatched rather than falling back to old ERP estimate fields.
  - UI/export should use backend `estimated_freight_inc_gst` instead of multiplying blindly because preferred InvoiceReader rows now store LSP booking freight as the comparison source, and CSV uploads may store system quote estimates that are already inc GST. Internal calculator totals are already `total_inc_gst`; only show that inc-GST total on outer lists/cards. Ex-GST Base/Surcharge/Fuel/GST belongs in breakdown/detail tables.
  ...
  - LSP package estimate sync command: `manage.py sync_lsp_package_estimates --full --batch-size 5000` for first load, then `manage.py sync_lsp_package_estimates --batch-size 5000` for incremental loads. It imports `data_raw.wms` + `data_raw.lsp` package bridge rows into `LspPackageEstimateSnapshot`.
  - LSP booking freight sync command: `manage.py sync_lsp_booking_orders --full --batch-size 5000` for first load, then `manage.py sync_lsp_booking_orders --batch-size 5000` for incremental loads. It imports `data_raw.lsp.lsp_booking_order.freight` and related tracking/order/reference fields into `LspBookingOrderSnapshot` for history/trace only.
  - New preferred reconciliation command: `manage.py sync_reconciliation_snapshots`. Default flow imports InvoiceReader charge snapshots for traceability, imports `InvoiceOrderMatchSnapshot` from `dbo.erp_match_results`, then creates `InvoiceReconciliationBatch` / `InvoiceReconciliationItem` from InvoiceReader's mapped ERP/order result and local `LspPackageEstimateSnapshot.lsp_estimated_freight`.
  ...
  - On 2026-06-25 LSP package estimates were fully synced: 366,178 `LspPackageEstimateSnapshot` rows, 240,450 with usable estimates, 319,668 linked to local orders. Reconciliation was rebuilt as batch id `252`: 900,934 rows, all basis `LSP_PACKAGE_ESTIMATE_FREIGHT`, 96,953 rows with matched package estimate, status counts MATCHED/OK 31,943, EXCEPTION/OVERCHARGE 44,937, EXCEPTION/UNDERCHARGE 20,073, UNMATCHED 803,981. The previous `LSP_BOOKING_ORDER_FREIGHT` batch is obsolete.
