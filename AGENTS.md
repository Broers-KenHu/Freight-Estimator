...
- InvoiceReader:
  ...
  - `InvoiceReconciliationItem.actual_freight` for the preferred path is `InvoiceOrderMatchSnapshot.amount_inc_gst` from InvoiceReader. `estimated_freight` now prefers `InvoiceOrderMatchSnapshot.erp_carrier_freight` from `dbo.erp_match_results` with basis `ERP_MATCH_RESULT_INC_GST`; this value is used directly for comparison and is not multiplied by GST. Only when `erp_carrier_freight` is missing should the command fall back to the local mapped `HistoricalOrder.postage_shipping_estimated_amount` / `source_estimated_freight` with basis `ERP_ORDER_EX_GST`, and that fallback comparison uses `estimated_freight * 1.10`. `system_estimated_freight` is CourieDelivery System Est. calculated by the current quote engine/rate cards and is already inc GST.
  - GST basis check on 2026-06-25: local `actual_freight` equals InvoiceReader `detail_amount_inc_gst` for 863,436 mapped rows; it must be labelled/displayed as Actual inc GST. User correction on 2026-06-25: if `erp_match_results.erp_carrier_freight` exists, treat it as the authoritative ERP carrier freight estimate and use it directly.
  ...
  - ERP estimate scope is currently `ORDER`: the preferred InvoiceReader path reads ERP Est. from `InvoiceOrderMatchSnapshot.erp_carrier_freight` when present, otherwise from the mapped `HistoricalOrder.postage_shipping_estimated_amount` then `HistoricalOrder.source_estimated_freight`. Do not compare a single tracking quote directly to an order-level ERP Est. unless the source table has already supplied the carrier freight value for that matched invoice/order row. In `CONSIGNMENT` mode the matrix command quotes each tracking/consignment separately, then aggregates totals per carrier/channel back to one owner-order row before calculating variance to ERP Est.
  - UI/export should use backend `estimated_freight_inc_gst` instead of multiplying blindly because preferred InvoiceReader rows can store ERP Match Result inc GST, fallback ERP order values can need GST conversion, and CSV uploads may store system quote estimates that are already inc GST. Internal calculator totals are already `total_inc_gst`; only show that inc-GST total on outer lists/cards. Ex-GST Base/Surcharge/Fuel/GST belongs in breakdown/detail tables.
