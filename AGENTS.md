...
- Orders:
  ...
  - Invoice reconciliation should match InvoiceReader actual charges to ERP shipment snapshots by tracking number, then use carrier/channel/service text only to disambiguate duplicate tracking candidates. The matched ERP shipment supplies ERP order no, third-party/rd3 order no, platform order no, platform, carrier, warehouse, and saved shipping estimate.
- Carriers:
  - Source: `data_raw.lsp`
...
- InvoiceReader:
  ...
  - Sync command: `manage.py sync_invoices_from_sqlserver`.
  ...
  - Matching order for the preferred path: `InvoiceChargeSnapshot.tracking_no` to `ErpShipmentSnapshot.tracking_no`, then carrier/channel/service scoring if multiple ERP rows share the same tracking. Do not rely on `fact_invoice_order_normalized`.
  - Once matched by shipment tracking, `InvoiceReconciliationItem.order_no` should be filled from `ErpShipmentSnapshot.erp_order_no` so the UI can show the ERP order number and saved estimate.
  - `InvoiceReconciliationItem.estimated_freight` is ERP Est. from ERP owner order fields. `system_estimated_freight` is CourieDelivery System Est. calculated by the current quote engine/rate cards.
  ...
  - Freight Audit Matrix command: `manage.py build_freight_audit_matrix --batch-id 221 --source-config HUNTER --mode CONSIGNMENT --limit 5000 --order-batch-size 5000`. `--limit` is distinct ERP owner-order count, not invoice row count. Remove `--limit` to process every matching owner order, but this can be very slow because every order runs all enabled quote channels and stores traceable quote runs/results.
  - ERP estimate scope is currently `ORDER`: `ErpShipmentSnapshot.estimated_freight` comes from `hpoms_owner_order.postage_shipping_estimated_amount` / `shipping_estimated_amount`. Do not compare a single tracking quote directly to ERP Est. In `CONSIGNMENT` mode the command quotes each tracking/consignment separately, then aggregates totals per carrier/channel back to one owner-order row before calculating variance to ERP Est.
  - ERP freight estimate values imported from ERP are ex GST. UI must display ERP Est as inc GST (`ERP Est * 1.10`). Internal calculator totals are already `total_inc_gst`; only show that inc-GST total on outer lists/cards. Ex-GST Base/Surcharge/Fuel/GST belongs in breakdown/detail tables.
  ...
  - Freight Audit Matrix intentionally calls `QuoteEngine.quote_selected_channels`, bypassing platform/warehouse carrier eligibility so that all enabled real calculators (currently PostageCalculator/approved local Hunter/Allied table channels) can be compared side by side for the same historical order. API channels will appear once their `QuoteChannel` and calculator/API integration are configured.
  - New preferred command: `manage.py sync_reconciliation_snapshots`. It builds `InvoiceChargeSnapshot`, syncs matching `ErpShipmentSnapshot` rows by invoice tracking, then creates `InvoiceReconciliationBatch` / `InvoiceReconciliationItem` from tracking-based snapshot matching.
  - Default `sync_reconciliation_snapshots` flow imports InvoiceReader charges first, then creates a temporary tracking table in the ERP/data_raw connection and syncs only ERP shipment rows that appear in the invoice snapshot. This avoids scanning/syncing all ~4.8M ERP shipment rows for every reconciliation run.
  ...
  - Useful modes: `--erp-only`, `--erp-from-invoices-only`, `--invoice-only`, `--reconcile-only`, `--clear-snapshots`, `--clear-reconciliation`, `--source-config HUNTER`, `--limit 100`, `--dry-run`.
...
& ..\.venv\Scripts\python.exe manage.py build_freight_audit_matrix --batch-id 221 --source-config HUNTER --mode CONSIGNMENT --limit 5000 --order-batch-size 5000
